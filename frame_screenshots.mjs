...
const outputDir = path.resolve('h:/learn-quran-kids-1/google-play-assets/framed-screenshots');
if (!fs.existsSync(outputDir)) {
  fs.mkdirSync(outputDir, { recursive: true });
}

const screenshotsDir = path.resolve('h:/learn-quran-kids-1/google-play-assets/screenshots');
...
const screens = [
  ...
  { file: 'screenshot-05-parent-dashboard.png', text: 'لوحة متكاملة لمتابعة إنجازات الوالدين' }
];

async function generateFramedScreenshots() {
  console.log('Launching browser...');
  const browser = await chromium.launch();
  
  for (let i = 0; i < screens.length; i++) {
    const screen = screens[i];
    const srcPath = path.join(screenshotsDir, screen.file);
    ...
    // Wait a brief moment to ensure fonts are loaded
    await page.waitForTimeout(1000);
    
    const outputPath = path.join(outputDir, 'framed-' + screen.file);
    await page.screenshot({ path: outputPath });
    console.log(`✅ Created framed screenshot: ${outputPath}`);
...
generateFramedScreenshots().catch(console.error);
