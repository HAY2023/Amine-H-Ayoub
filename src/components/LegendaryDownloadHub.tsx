...
export default function LegendaryDownloadHub() {
  ...
  const platforms = [
    ...
    { name: "Android", icon: <Smartphone className="w-6 h-6" />, desc: "هواتف وأجهزة أندرويد (APK)", url: `${HF_BASE}/Quran_1.0.0-001_Android.apk`, color: "from-emerald-500 to-emerald-700" },
    { name: "Windows", icon: <Monitor className="w-6 h-6" />, desc: "كمبيوتر ولابتوب (EXE)", url: `${HF_BASE}/Quran_1.0.0-001_x64-setup.exe`, color: "from-blue-500 to-blue-700" },
    { name: "Smart TV", icon: <Tv className="w-6 h-6" />, desc: "شاشات Android TV (APK)", url: `${HF_BASE}/Quran_1.0.0-001_Android_TV.apk`, color: "from-purple-500 to-purple-700" },
  ];
