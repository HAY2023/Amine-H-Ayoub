...
const QuranViewer = () => {
  ...
  return (
    <div className="min-h-screen page-nour text-foreground" dir="rtl">
      <header className="bg-card border-b border-border p-4 sticky top-0 z-50 shadow-soft">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-bold text-gradient-gold">📖 القرآن الكريم</h1>
          <Link
            to="/upload"
            aria-label="الذهاب إلى صفحة رفع الملفات"
            title="رفع صوت وصور"
            className="px-4 py-2 btn-gold rounded-lg text-sm font-bold transition"
          >
            📤 رفع
          </Link>
        </div>
      ...
      <div className="max-w-7xl mx-auto p-4 grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="md:col-span-1 card-nour rounded-xl overflow-hidden flex flex-col max-h-96 md:max-h-full">
          <div className="p-4 border-b border-border">
            <h2 className="text-lg font-bold text-foreground mb-3">📚 السور</h2>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-3 pr-10 py-2 bg-secondary border border-border rounded-lg text-secondary-foreground text-sm focus:outline-none focus:border-accent"
              />
            </div>
          </div>
        ...
        <div className="md:col-span-4 space-y-4">
          <div className="card-nour rounded-xl overflow-hidden flex flex-col animate-fade-up">
            <div className="bg-muted border-b border-border p-4 md:p-6 text-center">
              <h1 className="text-2xl md:text-3xl font-bold text-gradient-gold">سورة {currentSurah?.name}</h1>
              <p className="text-muted-foreground text-sm mt-1">عدد الآيات: {currentSurah?.ayahCount}</p>
            </div>

            <div className="flex-1 overflow-y-auto p-6 md:p-8 text-center min-h-64">
              <div style={{ fontSize: `${fontSize}px` }} className="leading-relaxed text-foreground font-quran">
                <p className="text-accent mb-6">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</p>
                <p className="text-muted-foreground">[نص السورة {currentSurah?.name}]</p>
              </div>
            </div>
            ...
            <div className="border-t border-border p-4 flex items-center justify-center gap-2">
              <button
                ...
                className="px-3 py-1 bg-secondary text-secondary-foreground hover:brightness-95 rounded text-sm"
              >
                −
              </button>
              <span className="text-sm text-muted-foreground min-w-12" aria-live="polite">{fontSize}px</span>
              <button
          ...
          <div className="card-nour rounded-xl p-6">
            <audio
              ref={audioRef}
            ...
            {audioError && (
              <div className="mb-4 p-3 bg-destructive/10 border border-destructive/40 rounded-lg flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-destructive flex-shrink-0" />
                <div>
                  <p className="text-destructive text-sm">{audioError}</p>
                  <p className="text-destructive/80 text-xs">الملف: {audioUrl}</p>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className="p-2 hover:bg-secondary disabled:opacity-50 rounded-lg"
              >
